...
{ TSQLMonitor }
  ...
  protected
    ...
    procedure DoOnTrace(Desc: pSQLTraceDesc; LogTrace: Boolean); virtual;
  public
...
{ TSQLConnection }
  ...
  private
    ...
    function OpenSchemaTable(eKind: TSchemaType; SInfo: string; SQualifier: string = ''; SPackage: string = ''): TCustomSQLDataSet;
...
procedure TSQLMonitor.DoOnTrace(Desc: pSQLTraceDesc; LogTrace: Boolean);
begin
...
begin
  ...
  try
    CheckActive;
    SetCursor(HourGlassCursor);
    DataSet := OpenSchemaTable(eKind, sInfo, '', PackageName);
    if Assigned(DataSet) then
    begin
      case eKind of
        stColumns:
...
begin
  DataSet := nil;
  try
    DataSet := OpenSchemaTable(stProcedureParams, ProcedureName,'', PackageName);
...
begin
  ...
  begin
    ...
    FSQLConnection.GetProcedureParams(CommandText, FSchemaInfo.PackageName, FProcParams);
  end;
  Result := FProcParams;
end;
...
begin
  List := TParams.Create;
{$IFDEF LINUX}
      if (not SQLConnection.Connected) and (csDesigning in ComponentState) and 
         (SQLConnection.LoginPrompt) then
        DatabaseError(SCannotCreateParams);
{$ENDIF}        
  try
    try
      ...
      FSQLConnection.GetProcedureParams(CommandText, FSchemaInfo.PackageName, ProcParams);
