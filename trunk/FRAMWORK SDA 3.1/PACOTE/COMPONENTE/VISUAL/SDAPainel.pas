...
uses
  ...
  ExtCtrls, SDAUtilitarios;

type
  TSDAPainel = class(TCustomPanel)
  private
...
constructor TSDAPainel.Create (AOwner: TComponent);
begin
...
procedure TSDAPainel.SetColors (Index: Integer; Value: TColor);
begin
  case Index of
...
procedure TSDAPainel.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TSDAPainel.CMTextChanged(var Message: TWmNoParams);
begin
  inherited;
  Invalidate;
end;

procedure TSDAPainel.SetTransparent(const Value: Boolean);
begin
  FTransparent := Value;
  Invalidate;
end;
