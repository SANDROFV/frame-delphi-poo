...
type
  TSDAScreenCanvas = class(TCanvas)
  private
    FDeviceContext: HDC;
  protected
...
uses
  SysUtils, Messages, Consts, Math, Variants,
...
begin
  if LongRec(ResID).Hi = 0 then
    ...
    S := StrPas(ResID);
  raise EResNotFound.Create(S)//CreateFmt(ResStr(SResNotFound), [S]);
end;
...
type
  TSDAParentControl = class(TWinControl);
...
begin
  ...
  try
    ...
    try
      ...
      with TSDAParentControl(Control.Parent) do
      begin
...
begin
  ...
  MessageDlg('Not Implemented', mtInformation, [mbOk], 0);
  Abort;
end;
...
begin
  ...
  WriteText(TSDAHack(Control).Canvas, ARect, MinOffs, H, S, Align, WordWrap);
end;
...
begin
  ...
  TSDAHack(Control).Canvas.Draw(Rect.Left, Rect.Top, Bmp);
end;

destructor TSDAScreenCanvas.Destroy;
begin
  FreeHandle;
  inherited Destroy;
end;

procedure TSDAScreenCanvas.CreateHandle;
begin
...
procedure TSDAScreenCanvas.FreeHandle;
begin
  if FDeviceContext <> 0 then
  begin
    Handle := 0;
...
procedure TSDAScreenCanvas.SetOrigin(X, Y: Integer);
var
  FOrigin: TPoint;
begin
